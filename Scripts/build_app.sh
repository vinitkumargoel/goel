...
# macOS kills a process sending an Apple event with no NSAppleEventsUsageDescription; the ATS dict admits user-supplied http:// URLs.
...
# Without this rpath the shipped binary resolves Sparkle from the SwiftPM build dir, not the bundle.
...
if [ "${BUNDLE_FFMPEG:-1}" = "1" ]; then
  ...
  # Not `[ -e … ] && codesign …`: under set -e a false test as the if-body's last statement aborts the build.
...
# check_min_os.sh exit 3 means the gate was waived; MINOS_OK=0 then blocks notarizing and shipping.
minos_gate() {
...
if [ "$GOEL_RELEASE" = "1" ]; then
  if [ -n "${CODESIGN_IDENTITY:-}" ]; then
    ...
    # Never `head -1` the whole list — line 1 is usually an Apple Development cert.
...
if [ -n "${CODESIGN_IDENTITY:-}" ]; then
  ...
  # Not `codesign --verify … && echo ok`: a non-final failure in an AND list skips set -e, passing a broken seal.
  ...
  # spctl accepts a local build regardless, so only a `source=Notarized Developer ID` match proves it opens elsewhere.
