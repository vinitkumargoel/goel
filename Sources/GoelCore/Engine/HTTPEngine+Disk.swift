...
extension HTTPEngine {
    ...
    /// THROWS when the volume can't be queried: assuming unlimited space let multi-GB downloads start on a full disk.
    static func validateDiskSpace(
        ...
        #if os(Linux)
        // `volumeAvailableCapacityForImportantUsageKey` is macOS-only.
