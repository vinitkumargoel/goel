...
enum RemotePortalAssets {
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    try{if(location.search.indexOf('token=')>=0){history.replaceState(null,'',location.pathname+location.hash);}}catch(_){}
    ...
    function copyFallback(t){try{const a=document.createElement('textarea');a.value=t;a.setAttribute('readonly','');a.style.position='fixed';a.style.opacity='0';document.body.appendChild(a);a.select();const ok=document.execCommand('copy');a.remove();return ok;}catch(_){return false;}}
    function copied(ok){toast(ok?'Copied':'Couldn’t copy — select the text manually',ok?ICON.copy:ICON.warn);}
    // `navigator.clipboard` is undefined in a non-secure context — which is the
    // default LAN deployment (plain HTTP). Fall back to a selection copy, and only
    // claim success when something actually copied; a "Copied" toast over an empty
    // clipboard is a lie the user discovers only when they paste.
    function copy(t){if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(t).then(()=>copied(true)).catch(()=>copied(copyFallback(t)));return;}copied(copyFallback(t));}
    // A 403 is no longer only read-only mode: the add route also refuses an
    // internal-network target, an out-of-root save folder, and a cross-site POST.
    // Show what the server actually said instead of blaming read-only.
    async function api(path,opts){const r=await fetch(path,opts||{});if(r.status===401){location.href='/';throw new Error('auth');}if(r.status===403){let m='';try{m=(await r.text()).trim();}catch(_){}toast(m||'Change blocked',ICON.warn);throw new Error('ro');}if(!r.ok)throw new Error('http '+r.status);return r;}
    ...
    return '';}
    // ---- actions ----
    async function doAct(id,act){try{if(act==='pause')await post('/api/pause?id='+id);else if(act==='resume')await post('/api/resume?id='+id);else if(act==='retry')await post('/api/retry?id='+id);toast({pause:'Paused',resume:'Resumed',retry:'Retried'}[act]||'Done');refresh();}catch(_){}}
    ...
    // ---- network ----
    // Cached so the Add modal can render its interface picker without a round trip
    // every time; refreshed whenever the Settings pane or a modal opens.
    let NET=null;
    async function loadNet(){try{const r=await api('/api/network');NET=await r.json();}catch(_){NET=null;}return NET;}
    function netEligible(){return NET&&NET.adapters?NET.adapters.filter(a=>a.eligible):[];}
    function nicLine(a){return esc(a.label)+' <span style="color:var(--text-faint)">'+esc(a.ipv4||'no address')+'</span>'+(a.expensive?' <span class="chip chip-d">Metered</span>':'');}
    function netSection(){const el=netEligible();if(el.length<2)return '';
      return '<div class="fg" style="margin-top:14px"><label class="flabel">Network</label>'
        +'<select class="finput" id="addNetMode"><option value="auto">Automatic — '+(NET.aggregation?'split across every eligible interface':'use the system default route')+'</option><option value="split">Split this download across interfaces</option><option value="single">Send it all through one interface</option></select>'
        +'<div id="addNetOpts"></div>'
        +'<div class="fhint">This machine has '+el.length+' usable interfaces. Splitting only helps when each has its own upstream link — two adapters behind the same router share one pipe and are usually slower together than the faster one alone.</div></div>';}
    function netOpts(){const box=$('#addNetOpts');const m=$('#addNetMode');if(!box||!m)return;const el=netEligible();
      if(m.value==='split')box.innerHTML='<div id="addNetList" style="margin-top:8px">'+el.map(a=>'<label style="display:flex;align-items:center;gap:8px;font-size:12.5px;padding:3px 0;cursor:pointer"><input type="checkbox" value="'+esc(a.name)+'" checked> '+nicLine(a)+'</label>').join('')+'</div>';
      else if(m.value==='single')box.innerHTML='<select class="finput" id="addNetOne" style="margin-top:8px">'+el.map(a=>'<option value="'+esc(a.name)+'">'+esc(a.label)+(a.ipv4?' — '+esc(a.ipv4):'')+'</option>').join('')+'</select>';
      else box.innerHTML='';}
    // Collapse the picker to a NetworkSelection spec the API understands.
    function netSpec(){const m=$('#addNetMode');if(!m)return 'auto';
      if(m.value==='single'){const one=$('#addNetOne');return one?'single:'+one.value:'auto';}
      if(m.value!=='split')return 'auto';
      const on=$$('#addNetList input:checked').map(x=>x.value);
      if(!on.length){toast('Pick at least one interface',ICON.warn);return null;}
      if(on.length===1)return 'single:'+on[0];
      return on.length===netEligible().length?'aggregate':'aggregate:'+on.join(',');}
    // ---- add ----
    function openAdd(){$('#scrim').innerHTML='<div class="modal"><div class="mhead"><div class="mic">'+ICON.link+'</div><h3>Add download</h3></div><div class="mbody"><label class="flabel">URL, magnet, FTP or SFTP link</label><textarea class="finput" id="addUrl" placeholder="https://example.com/file.iso&#10;magnet:?xt=urn:btih:...&#10;sftp://user@host/path/file.zip"></textarea><div class="fhint">Paste multiple lines to batch-add. Torrents/files download to the server running Goel°.</div><div class="twocol"><div class="fg"><label class="flabel">Save to <span class="chip chip-w">Server folder</span></label><input class="finput" id="addFolder" placeholder="Default folder (leave blank)"></div><div class="fg" style="flex:0 0 130px"><label class="flabel">Priority</label><select class="finput" id="addPrio"><option value="normal">Normal</option><option value="high">High</option><option value="low">Low</option></select></div></div><div id="addNetWrap"></div><label style="display:flex;align-items:center;gap:8px;margin-top:14px;font-size:12.5px;cursor:pointer"><input type="checkbox" id="addPaused"> Add paused</label></div><div class="mfoot"><button class="btn" data-close>Cancel</button><button class="btn primary" id="addGo">Add to queue</button></div></div>';openModal();setTimeout(()=>$('#addUrl').focus(),50);loadNet().then(()=>{const w=$('#addNetWrap');if(!w)return;w.innerHTML=netSection();const m=$('#addNetMode');if(m)m.onchange=netOpts;});$('#addGo').onclick=async()=>{const url=$('#addUrl').value.trim();if(!url){toast('Enter a URL or magnet first');return;}const net=netSpec();if(net===null)return;try{const r=await api('/api/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:url,folder:$('#addFolder').value.trim(),priority:$('#addPrio').value,paused:$('#addPaused').checked,network:net})});const j=await r.json();closeModal();S.filter='all';switchView('library');toast((j.added||1)>1?('Added '+j.added+' downloads'):'Added to queue');if(j.refused>0)toast(j.refused+' refused — internal network address',ICON.warn);refresh();}catch(_){}};}
    ...
    function renderSettings(){const cur=document.documentElement.dataset.theme;$('#setPad').innerHTML='<div class="ph">Settings</div><div class="psub">These apply to your browser. Server options (port, sign-in, password) are managed in the desktop app under Settings → Web Access.</div>'+
      ...
      '<div class="card pd" id="netCard"><p class="fhint" style="padding:8px">Loading network\u2026</p></div>'+
      '<div class="card pd"><div class="srow"><div class="sinfo"><div class="sname">Sign out</div><div class="sdesc">End this browser session.</div></div><div class="sctl"><button class="btn" id="btnLogout">'+ICON.logout+' Sign out</button></div></div></div>';renderNetCard();}
    async function renderNetCard(){const box=$('#netCard');if(!box)return;const n=await loadNet();
      if(!n){box.innerHTML='<p class="fhint" style="padding:14px">Could not read the network configuration.</p>';return;}
      const el=netEligible();const sel=n.selected||[];
      let h='<div class="srow"><div class="sinfo"><div class="sname">Split downloads across interfaces <span class="chip '+(n.aggregation?'chip-w':'chip-d')+'">'+(n.aggregation?'On':'Off')+'</span></div><div class="sdesc">'+(el.length<2?'This machine has one usable interface, so there is nothing to split across.':'Opens connections on several interfaces at once. Faster only when each interface has its own upstream link \u2014 two adapters behind the same router share one pipe and are usually slower together than the faster one alone.')+'</div></div><div class="sctl">'+((S.write&&el.length>=2)?'<button class="btn'+(n.aggregation?'':' primary')+'" id="netToggle">'+(n.aggregation?'Turn off':'Turn on')+'</button>':'')+'</div></div>';
      if(n.aggregation&&n.reason)h+='<div class="srow"><div class="sinfo"><div class="sdesc" style="color:var(--red)">'+ICON.warn+' Not splitting right now \u2014 '+esc(n.reason)+'</div></div></div>';
      if(n.locked)h+='<div class="srow"><div class="sinfo"><div class="sdesc">GOEL_AGGREGATION is set in <b>/etc/goel/config</b>, so a change made here is reverted the next time the service restarts. Run <b>goel config set aggregation on|off</b> on the server to make it permanent.</div></div></div>';
      h+='<div class="srow"><div class="sinfo"><div class="sname">Interfaces</div><div class="sdesc">Ticked interfaces are the ones a split may use. Leave them all ticked to use every eligible one.</div></div></div><div style="padding:0 2px 12px">'+n.adapters.map(a=>'<label style="display:flex;align-items:center;gap:8px;font-size:12.5px;padding:4px 0;cursor:'+((S.write&&a.eligible)?'pointer':'default')+';opacity:'+(a.eligible?'1':'.55')+'"><input type="checkbox" data-nic="'+esc(a.name)+'"'+((sel.length===0||sel.indexOf(a.name)>=0)?' checked':'')+((S.write&&a.eligible)?'':' disabled')+'> '+nicLine(a)+(a.eligible?'':' <span class="chip chip-d">Unavailable</span>')+'</label>').join('')+'</div>';
      h+='<div class="srow"><div class="sinfo"><div class="sname">Connections per interface</div><div class="sdesc">More streams help on high-latency links and hurt against servers that throttle per connection.</div></div><div class="sctl"><select class="finput" id="netStreams" style="width:76px"'+(S.write?'':' disabled')+'>'+[1,2,3,4,5,6,7,8].map(i=>'<option'+(i===n.streamsPerAdapter?' selected':'')+'>'+i+'</option>').join('')+'</select></div></div>';
      if(S.write)h+='<div class="srow"><div class="sinfo"><div class="sdesc">Applies to new downloads; running ones keep the interfaces they started on.</div></div><div class="sctl"><button class="btn primary" id="netSave">Save</button></div></div>';
      box.innerHTML=h;
      const tg=$('#netToggle');if(tg)tg.onclick=()=>saveNet({aggregation:!n.aggregation});
      const sv=$('#netSave');if(sv)sv.onclick=()=>{const on=$$('#netCard input[data-nic]:checked').map(x=>x.dataset.nic);const all=n.adapters.filter(a=>a.eligible).length;saveNet({adapters:on.length===all?[]:on,streams:parseInt($('#netStreams').value,10)});};}
    async function saveNet(body){try{const r=await api('/api/network',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});NET=await r.json();toast('Network settings saved');renderNetCard();}catch(_){}}
