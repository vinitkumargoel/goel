...
enum RemotePortalAssets {
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    async function removeTask(id,data){try{await post('/api/remove?id='+id+'&data='+(data?1:0));if(S.sel===id){S.sel=null;renderDetail(null);}toast(data?'Removed with data':'Removed',ICON.trash);refresh();}catch(_){}}
    // ---- add ----
