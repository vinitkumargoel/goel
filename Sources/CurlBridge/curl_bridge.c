...
/* True interface egress — NOT source-IP bind. */
static int gcb_sockopt_cb(void *clientp, curl_socket_t curlfd, curlsocktype purpose) {
    ...
    (void)setsockopt(curlfd, IPPROTO_IP, IP_BOUND_IF, &ifindex, sizeof(ifindex));
#ifdef IPV6_BOUND_IF
    (void)setsockopt(curlfd, IPPROTO_IPV6, IPV6_BOUND_IF, &ifindex, sizeof(ifindex));
#endif
...
static size_t gcb_http_write_thunk(char *ptr, size_t size, size_t nmemb, void *ud) {
    ...
    size_t n = size * nmemb;
    /* Do not write redirect bodies or non-206 bodies into a ranged file slot. */
    if (ctx->http_status != 0 && ctx->http_status != 206) {
        return n; /* drain but discard */
    }
...
static size_t gcb_http_header_thunk(char *buffer, size_t size, size_t nitems, void *ud) {
    ...
    if (strncasecmp(line, "Content-Range:", 14) == 0) {
        ...
        if (slash && slash[1] && slash[1] != '*') {
            ...
            if (total > 0) ctx->content_range_total = (int64_t)total;
        }
        return n;
    }
...
static void gcb_host_of(const char *url, char *out, size_t out_sz) {
    out[0] = '\0';
    if (!url || !out_sz) return;
    const char *p = strstr(url, "://");
    p = p ? p + 3 : url;
    size_t i = 0;
    while (p[i] && p[i] != '/' && p[i] != ':' && p[i] != '?' && p[i] != '#' && i + 1 < out_sz) {
...
/* Resolve a possibly-relative Location against the current URL into `out`. */
static int gcb_resolve_location(const char *base, const char *loc, char *out, size_t out_sz) {
    ...
    /* Relative — append to directory of base */
...
GCBHTTPResult gcb_http_range(const char *url,
    ...
    gcb_host_of(url, origin_host, sizeof(origin_host));
    ...
    for (int hop = 0; hop < 10; hop++) {
        ...
        char hop_host[256];
        gcb_host_of(current, hop_host, sizeof(hop_host));
        int strip = (strcasecmp(hop_host, origin_host) != 0) || (origin_https && !gcb_is_https(current));
        ...
        total_written += ctx.bytes_written;
        result.code = (int)rc;
        ...
        char next_url[2048];
        int has_redirect = 0;
        if (rc == CURLE_OK && ctx.http_status >= 300 && ctx.http_status < 400 && ctx.location[0]) {
        ...
        if (rc == CURLE_ABORTED_BY_CALLBACK) return result;
